...
export class ReviewNetworkRuleModalComponent implements OnInit {
  ...
  private prepareRegularNetworkRule = () => {
    ...
    this.ruleTypeClass = MapConstant.colourMap[this.data.networkRule.cfg_type.toUpperCase()];
  };
  ...
  updateNetworkRule = () => {
    ...
    this.securityEventsService.updateNetworkRule(this.data.networkRule)
      .subscribe(
        response => {
