...
export class SensorActionButtonsComponent implements ICellRendererAngularComp {
  ...
  deleteSensor = (sensor) => {
    const dialogRef = this.dialog.open(ConfirmDialogComponent, {
      maxWidth: "700px",
      data: {
        message: `${this.translate.instant("dlp.msg.REMOVE_CFM")}` + this.sanitizer.sanitize(SecurityContext.HTML, this.shortenFromMiddlePipe.transform(sensor.name, 30))
      },
      disableClose: true
    });
    dialogRef.componentInstance.confirm.pipe(switchMap(() => {
      ...
      (res) => {
        ...
        this.notificationService.open(this.translate.instant("dlp.msg.REMOVE_OK"));
        // close dialog
      ...
      error => {
        if (!MapConstant.USER_TIMEOUT.includes(error.status)) {
          this.notificationService.open(
            this.utils.getAlertifyMsg(error.error, this.translate.instant("dlp.msg.REMOVE_NG"), false),
