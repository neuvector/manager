(function () {
  ...
  function LoginFormController(
    ...
    function activate() {
      $mdDialog.cancel();
      function loginInit() {
        original = JSON.parse($window.sessionStorage.getItem("from"));
        version = $window.localStorage.getItem("version");
        gpuEnabled = $window.localStorage.getItem("_gpuEnabled");
        clearToken();
        ...
        if (currUrl.includes(PROXY_VALUE)) {
          vm.isFromSSO = true;
      ...
      function doLogin() {
        $http
          .post(LOGIN_URL, {
            username: vm.account && vm.account.email ? $sanitize(vm.account.email) : "",
            password: vm.account && vm.account.password ? vm.account.password : ""
          })
          .then(function (response) {
