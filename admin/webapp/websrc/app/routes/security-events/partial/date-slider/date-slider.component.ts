...
export class DateSliderComponent implements OnInit {
  ...
  ngOnInit(): void {
    ...
    this.options = {
      floor: this.securityEventsService.cachedSecurityEvents[this.securityEventsService.cachedSecurityEvents.length - 1].reportedTimestamp * 1000,
  ...
  filterByDate = () => {
    ...
    this.securityEventsService.displayedSecurityEvents = this.securityEventsService.cachedSecurityEvents.filter(event => {
      return event.reportedTimestamp * 1000 >= this.minValue && event.reportedTimestamp * 1000 <= this.maxValue;
    });
