...
export class AddEditVerifiersModalComponent implements OnInit {
  ...
  ngOnInit(): void {
    if (this.data.opType === GlobalConstant.MODAL_OP.ADD) {
      ...
      console.log(this.data.verifier.verifier_type, MapConstant.VERIFIER_TYPE.KEYPAIR )
  ...
  updateVerifier = () => {
    ...
    this.signaturesService.updateVerifierData(
      Object.assign(this.addEditVerifierForm.value, {root_of_trust_name: this.data.rootOfTrustName}),
      this.data.opType
    ).subscribe(
      response => {
        ...
        if (this.addEditVerifierForm.value.verifier_type === MapConstant.VERIFIER_TYPE.KEYPAIR) {
          ...
          [this.data.opType === GlobalConstant.MODAL_OP.ADD ?
            {
              ...
              cert_issuer: this.addEditVerifierForm.value.cert_issuer || '',
              cert_subject: this.addEditVerifierForm.value.cert_subject || ''
            } :
