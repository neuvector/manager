...
export class AdvanceSettingModalComponent implements OnInit {
  ...
  submitClientMode = () => {
    ...
    state.adm_client_mode = this.clientModeFormGroup.controls.clientModeFormControl.value;
    this.submittingForm = true;
    this.admissionRulesService.updateAdmissionState({state: state})
      .pipe(
        finalize(() => {
          this.submittingForm = false;
        })
  ...
  testK8s = () => {
    ...
    this.admissionRulesService.doK8sTest()
      .pipe(
        finalize(() => {
          this.submittingTest = false;
      ...
      error => {
        ...
        if (error.status === GlobalConstant.STATUS_NOT_FOUND) {
          switch (error.error.code) {
            case GlobalConstant.ADMISSION.INTERNAL_ERR_CODE.CLUSTER_ROLE_NOT_CONFIG:
              this.testErrMsg = this.translate.instant(
                "partner.admissionControl.msg.CLUSTER_ROLE_NOT_CONFIG"
              );
              break;
            case GlobalConstant.ADMISSION.INTERNAL_ERR_CODE.WEBHOOK_NOT_CONFIG:
              this.testErrMsg = this.translate.instant(
