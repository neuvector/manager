(function () {
  ...
  function ComplianceAssetController(
    ...
    function activate() {
      ...
      const ELEM_CONTAINER_INFO = document.getElementById("containerInfo");
      Utils.dragElement(ELEM_CONTAINER_INFO);
      ...
      $scope.showWorkload = (id) => {
        ContainerFactory.getContainer(id).then((container) => {
          ...
          if (
            ...
            $scope.stateHtml = "";
            let displayState = Utils.getI18Name($scope.container.state.toLowerCase());
            let labelCode = colourMap[$scope.container.state.toLowerCase()];
          ...
          $scope.onHost = false;
          ELEM_CONTAINER_INFO.style.top = TOP_LEFT_FLOAT_TOP;
          ELEM_CONTAINER_INFO.style.left = TOP_LEFT_FLOAT_LEFT;
        });
      };
