          ...
          delete {
            Utils.respondWithNoCacheControl() {
              complete {
                ...
                paginationCacheManager[List[org.json4s.JsonAST.JValue]].removePagedData(s"$cacheKey-audit")
                paginationCacheManager[Array[ScannedWorkloads2]].removePagedData(s"$cacheKey-workload")
                paginationCacheManager[Array[GroupDTO]].removePagedData(s"$cacheKey-group")
                paginationCacheManager[List[org.json4s.JsonAST.JValue]].removePagedData(s"$cacheKey-network-rule")
              ...
              get {
                Utils.respondWithNoCacheControl() {
                  complete {
                    ...
                    RestClient.httpRequestWithHeader(s"${baseClusterUri(tokenId)}/user_role_permission/options",
                      GET,
                      "",
                      tokenId)
                  }
                    ...
                    Utils.respondWithNoCacheControl() {
                      complete {
                        ...
                        if (name.isEmpty) {
                          ...
                          url = s"user_role/${
                            UrlEscapers.urlFragmentEscaper().escape(name.get)}"
                        }
                        logger.info(s"Getting roles ...")
                        RestClient.httpRequestWithHeader(s"${baseClusterUri(tokenId)}/$url",
                          GET,
                          "",
                          tokenId)
                      }
                  ...
                  complete {
                    if (name.nonEmpty) {
                      try {
                        ...
                        val userWrap = jsonToUserWrap(Await.result(result, RestClient.waitingLimit.seconds))
                      ...
                      try {
                        ...
                        val userImage = users.users.map(userToUserImage).sortWith(_.username < _.username)
                        UsersOutput(
                          users.domain_roles,
                          users.global_roles,
                          userImage
                ...
                Utils.respondWithNoCacheControl() {
                  complete {
                    ...
                    try {
                      ...
                      val selfWrap = jsonToSelfWrap(Await.result(result, RestClient.waitingLimit.seconds))
