...
export class CertificateDeserializer {
    ...
    public getMainfest(cert: forge.pki.Certificate):  CertificateManifest {
        ...
        const daysLeft = moment.duration(moment(cert.validity.notAfter).diff(moment())).asDays();
        ...
        return {
            ...
            organizationUnit: cert.subject.getField(CertificateFieldKey.ORGANIZATION_UNIT),
    ...
    private getKeyAlgorithmType(cert: forge.pki.Certificate) {
        if ((cert.publicKey as forge.pki.rsa.PublicKey).n && (cert.publicKey as forge.pki.rsa.PublicKey).e) {
