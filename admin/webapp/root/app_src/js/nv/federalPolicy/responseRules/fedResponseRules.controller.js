(function() {
    ...
    angular
        .module("app.assets")
        .controller("FederalPolicyResponseRulesController", FederalPolicyResponseRulesController);
    FederalPolicyResponseRulesController.$inject = [
        "$rootScope",
        "$scope",
        "$state",
        "$location",
    ...
    function FederalPolicyResponseRulesController(
        $rootScope,
        $scope,
        $state,
        $location,
        ...
        $scope.graphHeight = $window.innerHeight - 300;
        ...
        $scope.isWriteResponseRuleAuthorized = AuthorizationFactory.getDisplayFlag("write_response_rule") && isFed;
        $scope.isConfigAdmCtrlAuthorized = $scope.isWriteResponseRuleAuthorized;

        responseRulesService.categories =
            $scope.summary.platform.toLowerCase().indexOf(KUBE) >= 0 && $scope.isConfigAdmCtrlAuthorized ?
                [
                    "security-event",
                    "event",
                    "cve-report",
        ...
        function activate() {
            ...
            let columnDefs = [
                ...
                },
                {
                    ...
                    cellRenderer: function(params) {
                        if (params.value) {
                            return `<div style="word-wrap: break-word;">${$sanitize(params.value)}</div>`;
                        }
                    },
            ...
            function actionsRenderFunc(params) {
              if ($scope.isWriteResponseRuleAuthorized) {
                return (
                    '<div class="rule-actions-expand fade-in-right">' +
                    '       <em class="fa fa-edit fa-lg mr-sm text-action"' +
                    '         ng-click="editResponseRule($event, data.id)" uib-tooltip="{{\'policy.TIP.EDIT\' | translate}}">' +
                    "       </em>" +
                    '       <em class="fa fa-plus-circle fa-lg mr-sm text-action" ' +
                    '         ng-click="addResponseRule($event, data.id)" uib-tooltip="{{\'policy.TIP.ADD\' | translate}}">' +
                    "       </em>" +
                ...
                return (
                    ...
                    '         ng-click="editResponseRule($event, data.id, true)" uib-tooltip="{{\'policy.editPolicy.VIEW\' | translate}}">' +
                    "       </em>" +
                    "     </div>" +
                    '     <div class="rule-actions-collapse">' +
                    '       <em class="fa fa-ellipsis-h fa-lg mr-sm text-action hand">' +
            ...
            function requestResponsePolicy() {
                $http
                    .get(RESPONSE_POLICY_URL, {params: {scope: "fed"}})
            ...
            function requestConditionOption() {
                ...
                $http
                    .get(CONDITION_OPTION_URL, {params: {scope: "fed"}})
                    .then(function(response) {
                        $scope.conditionOptions = response.data.response_rule_options;
                        $scope.webhookList = response.data.webhooks || [];
                        responseRulesService.conditionOptions =
                            response.data.response_rule_options;
            ...
            function requestContainerGroup() {
                ...
                $http
                    .get(GROUP_LIST_URL, {params: {scope: "fed", f_kind: "in,container|node"}})
                    .then(function(response) {
            ...
            $scope.onFilterChanged = function(value) {
                ...
                $scope.count =
                    filteredCount === $scope.rules.length || value === ""
                        ...
                        filteredCount
                        )} ${outOf} ${$scope.rules.length} ${getEntityName(
                        $scope.rules.length
                        )}`;
            };
        }
        $scope.reload();
        $scope.addResponseRule = function(ev, id) {
            let success = function() {
                ...
                $mdDialog
                    .show({
                        controller: DialogController4AddResponseRule,
                        templateUrl: "dialog.addResponseRule.html",
                        targetEvent: ev,
                        locals: {
                          conditionOptions: $scope.conditionOptions,
        ...
        $scope.editResponseRule = function(ev, id, isReadonly = false) {
            let success = function() {
                ...
                $mdDialog
                    .show({
                        controller: DialogController4EditResponseRule,
                        templateUrl: "dialog.editResponseRule.html",
                        targetEvent: ev,
                        locals: {
                          conditionOptions: $scope.conditionOptions,
                          webhookList: $scope.webhookList,
                    ...
                    .then(
                        function() {
                            responseRulesService.rules[
                                $scope.index4edit
                                ].conditions = responseRulesService.conditionTagToString(
                                responseRulesService.rules[$scope.index4edit].conditions
                            );
                            console.log("pre-reload: ", responseRulesService.rules);
                            let rowNode = $scope.gridOptions.api.getDisplayedRowAtIndex(
                                $scope.index4edit
                            );
                            let rowNodes = [];
        ...
        $scope.deleteRuleItem = function(event, id) {
            ...
            rowNode.setSelected(true);
            let confirmBox =
                $translate.instant("responsePolicy.dialog.content.REMOVE_CONFIRM") + $sanitize(id);
            console.log(
            ...
            Alertify.confirm(confirmBox).then(
                function toOK() {
                    ...
                    $http
                        .delete(`${RESPONSE_POLICY_URL}?scope=fed&id=${id}`)
                        .then(function(response) {
                            Alertify.set({ delay: ALERTIFY_SUCCEED_DELAY });
                            Alertify.success(
        ...
        $scope.toggleRuleItem = function(event, rule) {
            ...
            payload.config.cfg_type = CFG_TYPE.FED;
        ...
        DialogController4AddResponseRule.$inject = [
            "$scope",
            "$mdDialog",
            "$translate",
            "$sanitize",
        ...
        function DialogController4AddResponseRule(
            $scope,
            $mdDialog,
            $translate,
            $sanitize,
            ...
            $scope.changeCategory = function() {
                ...
                $scope.conditionPatternSample =
                    responseRulesService.conditionPatternSample[$scope.newRule.event];
                responseRulesService.conditionOptions = angular.copy(conditionOptions);
            ...
            $scope.tagAdding = function(tag) {
                console.log(responseRulesService.conditionOptions)
                console.log(
                    "Pattern: " +
                    responseRulesService.getPattern($scope.newRule.event, conditionOptions) +
                    "\n",
                    "Tag name: " + tag.name + "\n",
                    "Test result: " +
                    responseRulesService.getPattern($scope.newRule.event, conditionOptions).test(tag.name)
                );
                ...
                return responseRulesService
                    .getPattern($scope.newRule.event, conditionOptions)
                    .test(tag.name);
            };
            ...
            function makeAutoCompleteList() {
                ...
                function loadAll() {
                    let allGroup = responseRulesService.groupList
                        ? Utils.removeGroupExceptions(responseRulesService.groupList, "response")
                        : [];
        ...
        DialogController4EditResponseRule.$inject = [
            "$scope",
            "$mdDialog",
            "$translate",
            "$sanitize",
        ...
        function DialogController4EditResponseRule(
            $scope,
            $mdDialog,
            $translate,
            $sanitize,
            ...
            $scope.tagAdding = function(tag) {
                console.log(
                    "Pattern: " +
                    responseRulesService.getPattern($scope.rule.event, conditionOptions) +
                    "\n",
                    "Tag name: " + tag.name + "\n",
                    "Test result: " +
                    responseRulesService.getPattern($scope.rule.event, conditionOptions).test(tag.name)
                );
                ...
                initializeTagStyle();
                return responseRulesService
                    .getPattern($scope.rule.event, conditionOptions)
                    .test(tag.name);
            };
            ...
            $scope.editRule = function(ev) {
                ...
                payload.config.group = $sanitize(payload.config.group);
