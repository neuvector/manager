...
export class ActionButtonsComponent implements ICellRendererAngularComp {
  ...
  addNetworkRule = (id) => {
    const addEditDialogRef = this.dialog.open(AddEditNetworkRuleModalComponent, {
      width: "80%",
      data: {
        opType: GlobalConstant.MODAL_OP.ADD,
        networkRuleOptions: this.params.context.componentParent.networkRuleOptions,
        index: this.params.context.componentParent.networkRules.findIndex(rule => rule.id === id),
    ...
    const addEditDialogRef = this.dialog.open(AddEditNetworkRuleModalComponent, {
      ...
      data: {
        ...
        cfgType: this.params.context.componentParent.source === GlobalConstant.NAV_SOURCE.FED_POLICY
          ...
          : GlobalConstant.SCOPE.LOCAL,
        updateGridData: this.params.context.componentParent.updateGridData,
        selectedNetworkRule: this.params.context.componentParent.networkRules.filter(rule => rule.id === id)[0]
      },
      disableClose: true
    });
  };
  ...
  private toggleMaskingDeletedRow = (id: number, isMaskOn: boolean) => {
    let targetIndex = this.params.context.componentParent.networkRules.findIndex(rule => rule.id === id);
    let row = this.params.context.componentParent.gridOptions.api!.getDisplayedRowAtIndex(targetIndex);
    this.params.context.componentParent.networkRules[targetIndex].remove = isMaskOn;
    this.params.context.componentParent.gridOptions.api!.redrawRows({rowNodes: [row]});
  ...
  promoteNeworkRuleOnEntry = (id) => {
    ...
    this.networkRulesService.promoteNetworkRulesData(payload)
      .subscribe(
        res => {
          setTimeout(() => {
