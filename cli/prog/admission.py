...
OpsDisplay1 = {CriteriaOpContainsAny: 'is', CriteriaOpNotContainsAny: 'is not', CriteriaOpContainsAll: CriteriaOpContainsAll, CriteriaOpContainsOtherThan: CriteriaOpContainsOtherThan}
OpsDisplay2 = {CriteriaOpContainsAny: 'contains any in', CriteriaOpNotContainsAny: 'not contains any in', CriteriaOpContainsAll: 'contains all in', CriteriaOpContainsOtherThan: 'contains value not in'}
...
def show_admission_state(data):
    ...
    click.echo("  client mode:    {} ({})".format(state["adm_client_mode"], state["adm_client_mode_options"][state["adm_client_mode"]]))
...
@click.option("--scope", default="all", type=click.Choice(['fed', 'local', 'all']), help="Show federal, local or all admission rules")
@click.pass_obj
@click.pass_context
...
@click.option("--client_mode", required=False, help="The client mode that Kube-apiserver uses when sending requests to Neuvector admission control webhook server")
@click.pass_obj
...
def delete_admission_rule(data, id, scope):
    ...
    if id is not None:
        ...
        deleteRules = False
        if scope == "fed":
            if click.confirm('All federal-level admission control deny rules will be deleted. Do you want to continue?'):
                deleteRules = True
        else:
            if click.confirm('All local non-default admission control rules(except default rules) will be deleted. Do you want to continue?'):
                deleteRules = True
        if deleteRules:
            if scope == 'fed' or scope == 'local':
                args["scope"] = scope
...
def request_admission_test(data):
    ...
    click.echo("")
    click.echo("NeuVector admission control webhook server receives the request sent from Kube-apiserver for testing successfully.")
    click.echo("")
...
@click.option("--id",  multiple=True, help="id of the admission control rules to promote to federate level on master cluster")
@click.pass_obj
