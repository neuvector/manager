...
export class AddEditNetworkRuleModalComponent implements OnInit {
  ...
  constructor(
    ...
    this.filteredApps = this.addEditNetworkRuleForm.get('applicationsCtrl')!.valueChanges.pipe(
      ...
      map((app: string | null) => (
        app ?
        this.filter(app, this.excludesSelectedApps(this.data.networkRuleOptions.appList, this.applications)) :
        this.excludesSelectedApps(this.data.networkRuleOptions.appList, this.applications).slice())),
    );
    let enpointList =
      this.data.source === GlobalConstant.NAV_SOURCE.FED_POLICY ?
      this.data.networkRuleOptions.groupList.map(group => group.name) :
      this.data.networkRuleOptions.hostList.map(host => `Host:${host.name}`).concat(this.data.networkRuleOptions.groupList.map(group => group.name))
    this.filteredFromEndpoints = this.addEditNetworkRuleForm.get('from')!.valueChanges.pipe(
      startWith(''),
      map((from: string) => (
        this.filter(from, enpointList)
      ))
    );
    this.filteredToEndpoints = this.addEditNetworkRuleForm.get('to')!.valueChanges.pipe(
      startWith(''),
      map((to: string) => (
        this.filter(to, enpointList)
      ))
    );
  }
  ...
  private filter = (value: string, availableOptions: Array<string>): Array<string> => {
    const filterValue = value.toLowerCase();
    return availableOptions.filter(availableOption => availableOption.toLowerCase().includes(filterValue));
  };

  private excludesSelectedApps = (appOptionList: Array<string>, applications: Array<string>): Array<string> => {
    return appOptionList.filter(appOption => {
      return applications.map(app => app.toLowerCase()).indexOf(appOption.toLowerCase()) < 0;
    });
  };
  ...
  updateRule = () => {
    ...
    if (this.data.opType === this.modalOp.EDIT) {
      ...
      this.data.updateGridData([payload], this.data.index + 1, UpdateType.Insert);
    }
