    ...
    clientIP { ip =>
      entity(as[Password]) { userPwd =>
        def login: Route = {
          ...
          authToken = UserTokenNew(authToken.token, authToken.emailHash, authToken.roles, authToken.login_timestamp, suseCookieValue)
          logger.info("login with SUSE cookie: {} ", authToken.suse_auth_token)
