...
@click.option("--deploy_scan_result", default="disable", type=click.Choice(["enable", "disable"]),
              help="Deploy scan result to managed clusters")
@click.pass_obj
def request_federation_promote(data, name, server, port, use_proxy, deploy_scan_result):
    ...
    req["deploy_scan_data"] = False
    if deploy_scan_result == "enable":
        req["deploy_scan_data"] = True
...
def show_federation_config(data):
    ...
    if resp is None or (resp["fed_role"] != clusterRoleMaster):
        ...
        if "deploy_scan_data" in resp:
            cfg["deploy scan result"] = resp["deploy_scan_data"]
...
@click.option("--deploy_scan_result", default="disable", type=click.Choice(["enable", "disable"]),
              help="Deploy scan result to managed clusters")
@click.pass_obj
def set_federation_config(data, name, server, port, use_proxy, deploy_scan_result):
    ...
    if name != None and name != "":
        body["name"] = name
    body["deploy_scan_data"] = False
    if deploy_scan_result == "enable":
        body["deploy_scan_data"] = True
