...
import {Cluster, ClusterData, ClusterSummary, ErrorResponse} from "@common/types";
...
export class MultiClusterComponent implements OnInit {
  ...
  initialize(fedData: ClusterData): void {
    ...
    this.isOrdinary = fedData.fed_role.length === 0;
    this.isFederal = fedData.fed_role.length > 0;
    this.clusterData = fedData;

    this.clusterCount = this.clusterData.clusters ? this.clusterData.clusters.length: 0;
  ...
  getSummary(id?:String){
    if(id){
      this.multiClusterService.getRemoteSummary(id).subscribe( (summary:any) => { console.log("summary:", summary)}
      );
    }else{
  ...
  pageInit() {
    ...
    this.multiClusterService.getClusters()
      .pipe(
        finalize(() => {
          this.loaded = true;
        })
  ...
  showJoiningDialog(): void {
    ...
    dialogRef.afterClosed().subscribe(result => {
      console.log("dialog closed and returns ", result);
    });
  }
